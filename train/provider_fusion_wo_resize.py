...
class FrustumDataset(object):
    ...
    def get_bbox(self, box,img_h,img_w):
        xmin, ymin, xmax, ymax = box
        xmax += 1
        ymax += 1
        x_b = xmax - xmin
        for tt in range(len(border_list)):
            if x_b > border_list[tt] and x_b < border_list[tt + 1]:
                x_b = border_list[tt + 1]
                break
        y_b = ymax - ymin
        for tt in range(len(border_list)):
            if y_b > border_list[tt] and y_b < border_list[tt + 1]:
                y_b = border_list[tt + 1]
                break
        center = [int((xmin + xmax) / 2), int((ymin + ymax) / 2)]
        xmin = center[0] - int(x_b / 2)
        xmax = center[0] + int(x_b / 2)
    ...
    def __getitem__(self, index):
        ...
        if self.with_image:
            ...
            xmin, xmax, ymin, ymax = self.get_bbox(box,image.shape[0],image.shape[1])

            image_crop = image[ymin:ymax, xmin:xmax, :]#(h, w, 3)
        ...
        if self.from_rgb_detection:
            ...
            if self.with_image:
                data_inputs.update({'image': self.norm(torch.FloatTensor(image_crop))})
